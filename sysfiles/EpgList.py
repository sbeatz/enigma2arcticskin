...
#					pass
	...
	def buildSingleEntry(self, service, eventId, beginTime, duration, EventName):
		...
		if config.skin.primary_skin.value == "Arctic/skin.xml":
			...
			(eListboxPythonMultiContent.TYPE_TEXT, r1.left(), r1.top()+7, r1.width(), r1.height(), 0, RT_HALIGN_RIGHT, self.days[t[6]]),
			(eListboxPythonMultiContent.TYPE_TEXT, r2.left(), r2.top()+7, r2.width()+20, r1.height(), 0, RT_HALIGN_RIGHT, "%02d.%02d.  %02d:%02d"%(t[2],t[1],t[3],t[4]))
		]		
		else:
			res = [
			None, # no private data needed
		...
		if rec:
			if config.skin.primary_skin.value == "Arctic/skin.xml":
				res.extend((
					(eListboxPythonMultiContent.TYPE_PIXMAP_ALPHATEST, r3.left()+20, r3.top()+12, 21, 21, clock_pic),
					(eListboxPythonMultiContent.TYPE_TEXT, r3.left() + 55, r3.top() + 7, r3.width(), r3.height(), 0, RT_HALIGN_LEFT, EventName)
				))
			else:
				res.extend((
			...
			if config.skin.primary_skin.value == "Arctic/skin.xml":
				res.append((eListboxPythonMultiContent.TYPE_TEXT, r3.left()+20, r3.top()+7, r3.width(), r3.height(), 0, RT_HALIGN_LEFT, EventName))
			else:
...
#				print "end", end
	...
	def buildEPGBarEntry(self, service, eventId, beginTime, duration, EventName, nowTime, service_name):
		...
		if rec:
			res.extend((
				(eListboxPythonMultiContent.TYPE_PIXMAP_ALPHATEST, 5, 0, 21, 21, clock_pic),
				res.append((eListboxPythonMultiContent.TYPE_TEXT, 30, 0, r0.width() - 30, r0.height(), 0, RT_HALIGN_LEFT, epg_str))
			))
		else:
			res.append((eListboxPythonMultiContent.TYPE_TEXT, 5, 0, r0.width() - 5, r0.height(), 0, RT_HALIGN_LEFT, epg_str))
		return res
