...
from config import KEY_LEFT, KEY_RIGHT, KEY_HOME, KEY_END, KEY_0, KEY_DELETE, KEY_BACKSPACE, KEY_OK, KEY_TOGGLEOW, KEY_ASCII, KEY_TIMEOUT, KEY_NUMBERS, ConfigElement, ConfigText, ConfigPassword, ConfigNumber
...
class ConfigListScreen:
	...
	def handleInputHelpers(self):
		if self["config"].getCurrent() is not None:
			can_have_help_screen = False
			if isinstance(self["config"].getCurrent()[1], ConfigText) and not isinstance(self["config"].getCurrent()[1], ConfigNumber) and not isinstance(self["config"].getCurrent()[1], ConfigPassword):
				can_have_help_screen = True
				self["VirtualKB"].setEnabled(True)
				if self.has_key("VKeyIcon"):
					self["VKeyIcon"].boolean = True
			elif isinstance(self["config"].getCurrent()[1], ConfigPassword):
				can_have_help_screen = True
				if self.has_key("VKeyIcon"):
					self["VirtualKB"].setEnabled(True)
					self["VKeyIcon"].boolean = True
			...
			if can_have_help_screen and self.has_key("HelpWindow"):
